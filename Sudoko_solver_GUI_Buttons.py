...
matrix_live = copy.deepcopy(matrix[:][:])
def check(number,row,column,matrix1):
    ...
    return True
def live_check(i,j):
    global matrix_live,matrix
    matrix_live[i][j] = int(eval("row"+str(i)+"column"+str(j))["text"])
    if (matrix_live[i][j] < 9):
        matrix_live[i][j] = matrix_live[i][j] + 1
    else:
        matrix_live[i][j] = 0
    for _ in range(0,9):
        for __ in range(0,9):
            if(matrix[_][__] == 0):
                if(check(matrix_live[_][__],_,__,matrix_live) and matrix_live[_][__] != 0):
                    eval("row"+str(_)+"column"+str(__)).configure(bg="#00FF00")
                else:
                    eval("row" + str(_) + "column" + str(__)).configure(bg="#FA5858")
    # if(check(matrix_live[i][j],i,j,matrix_live)):
    #     eval("row"+str(i)+"column"+str(j)).configure(bg="Green")
    # else:
    #     eval("row" + str(i) + "column" + str(j)).configure(bg="Red")
